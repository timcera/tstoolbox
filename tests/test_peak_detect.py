...
class TestPeakDetect(TestCase):
    ...
    def test_peak_rel(self):
        out = capture(tstoolbox.peak_detection, input_ts=self.ats, print_input=True, type='both')
        self.maxDiff = None
        self.assertEqual(out, test_sinwave)

    def test_peak_minmax(self):
        out = capture(tstoolbox.peak_detection, method='minmax', window=3, input_ts=self.ats, print_input=True, type='both')
        self.maxDiff = None
        self.assertEqual(out, test_sinwave)
